...
const MedicineSearch = () => {
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        const response = await api.get(`/medicine/?search=${query}`);
  ...
  const handleSelect = (medicine) => {
    ...
    const updated = [
      medicine.name,
      ...recentSearches.filter((item) => item !== medicine.name),
    ].slice(0, 5);
    ...
    <div className="min-h-[calc(100vh-80px)] bg-gradient-to-br from-green-100 via-blue-50 to-purple-100 justify-center pt-12 px-6">
      <div className="bg-white bg-opacity-80 backdrop-blur-lg shadow-lg rounded-2xl w-full max-w-2xl p-8 mx-auto">
        <div className="text-center mb-6">
          <h1 className="text-4xl font-extrabold text-blue-800">Medicine Search</h1>
          <p className="text-gray-600 mt-1">Find details like usage & manufacturer</p>
        </div>
        ...
        {showSuggestions && (
          <ul className="bg-white border rounded shadow mt-2 divide-y max-h-64 overflow-auto z-10 relative">
            ...
            {query.trim() !== "" && suggestions.map((med, index) => (
              <li
                key={index}
                ref={(el) => (itemRefs.current[index] = el)}
                className={`p-2 cursor-pointer transition ${
        ...
        {selectedMedicine && selectedMedicine.name && (
          <div className="bg-white mt-6 p-4 rounded-lg shadow-md border border-blue-100">
            ...
            <p>
              ...
              {selectedMedicine.usage || "Unknown"}
            </p>
            <p>
              <span className="font-semibold">Manufacturer:</span>{" "}
              {selectedMedicine.manufacturer || "Unknown"}
            </p>
          </div>
        )}
      </div>
